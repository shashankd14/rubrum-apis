...
public interface SalesOrderJswRepository extends JpaRepository<SalesOrderJswEntity, Integer> {
	...
	@Query(value = "SELECT so.so_id, so.so_number, so.socreatedate, so.deliverymethod, "
			...
			" FROM jsw_sales_order so INNER JOIN jsw_sales_order_child so_child ON so_child.so_id = so.so_id INNER JOIN jsw_branch_master br ON br.branch_id = so.branch_id LEFT JOIN jsw_material_master mm ON mm.mm_id = so_child.mm_id" +
