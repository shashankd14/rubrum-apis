...
public interface SalesOrderAllocationJswRepository extends JpaRepository<SalesOrderAllocationEntity, Integer> {
	...
	@Query(value = "select customerid, salesorder_id, item_id, allo.allocated_soqty, "
			...
			+ " (SELECT wh1.ware_house_name FROM jsw_warehouse_master wh1 where wh1.ware_house_id=chld.wearhouse_id) itemWhName "
			...
			+ " and allo.inward_entry_id=inw.inwardentryid "
			+ " and allo.so_child_id in (select distinct allocat.so_child_id from jsw_sales_order_allocation allocat where allocat.so_allocation_id = :soAllocationId) ", 
		nativeQuery = true)
