...
public interface LocationMasterRepository extends JpaRepository<LocationMasterEntity, Integer> {
	...
	@Query("SELECT l FROM LocationMasterEntity l WHERE (l.isDeleted = false OR l.isDeleted IS NULL) ORDER BY l.id DESC")
	List<LocationMasterEntity> findAllActive();
	
	@Query("SELECT l FROM LocationMasterEntity l "
		+ " WHERE (:locationIds IS NULL OR l.locationId IN :locationIds) "
		+ " AND (l.isDeleted = false OR l.isDeleted IS NULL) " 
		+ " ORDER BY l.id DESC")
	List<LocationMasterEntity> findAllLocations(@Param("locationIds") List<Integer> locationIds);
