...
public interface DeliveryDetailsRepository extends JpaRepository<DeliveryDetails, Integer> {
    ...
    public Page<DeliveryDetails> findAllDeliveries(@Param("searchText") String searchText, @Param("partyId") int partyId, @Param("userIds") List<Integer> userIds, Pageable pageable);
	...
	public Page<DeliveryDetails> findAllDeliveries(@Param("searchText") String searchText,
			@Param("partyIds") List<Integer> partyIds, @Param("userIds") List<Integer> userIds, Pageable pageable);
    
    @Query("select dd from DeliveryDetails dd join dd.instructions ins join ins.inwardId inw where dd.createdBy in (:userIds) and dd.isDeleted is false and ins.deliveryDetails is not null and "
    		+ " ( inw.coilNumber like %:searchText% or inw.customerBatchId like %:searchText% or "
    		+ " inw.customerInvoiceNo like %:searchText% or inw.party.partyName like %:searchText% ) group by inw, dd")
    public Page<DeliveryDetails> findAllDeliveries(@Param("searchText") String searchText, @Param("userIds") List<Integer> userIds, Pageable pageable);
