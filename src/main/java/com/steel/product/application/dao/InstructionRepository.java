...
//    public List<PartInstruction> findPartDetailsFetchInstructions(@Param("partDetailsId")String partDetailsId);
	...
	@Query("select ins from Instruction ins left join fetch ins.inwardId left join fetch ins.parentInstruction where  ins.isDeleted= 0 and ins.inwardId.inwardEntryId = :inwardId and ins.status.statusId = :statusId")
