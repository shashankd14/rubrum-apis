...
public interface SalesOrderRepository extends JpaRepository<SalesOrderEntity, Integer> {
	...
	@Query(value = "select packet_id, inwardid, coilnumber, customerbatchid, materialgrade, materialdesc, fthickness, fweight, npartyid,partyname,fWidth, flength,fquantity, "
			...
			+ " (SELECT so.so_number from salesorder so, salesorder_child sochild where so.so_id=sochild.so_id and sochild.instruction_id = child.instructionid limit 1) as sono,"
			+ " (SELECT so.customer_code_id from salesorder so, salesorder_child sochild where so.so_id=sochild.so_id and sochild.instruction_id = child.instructionid limit 1) as customer_code"
	...
	@Query(value = "SELECT distinct so.so_number,inwardid "
			+ " FROM product_tblinwardentry parent, product_instruction child, salesorder_child so_child, salesorder so, product_tblpartydetails party "
	...
	@Query(value = "select packet_id, inwardid, coilnumber, customerbatchid, materialgrade, materialdesc, fthickness,  weight, npartyid,partyname,width, length, "
			+ "process_status, instruction_status,(select so_number from salesorder so where so.so_id= a.so_id) sonumber, "
