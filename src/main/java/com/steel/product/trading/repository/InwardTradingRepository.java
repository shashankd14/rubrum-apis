...
public interface InwardTradingRepository extends JpaRepository<InwardTradingEntity, Integer> {
	...
	@Query(value = "SELECT inward.inward_id, inward.inward_number"
			+ " FROM trading_inward inward, trading_vendor_master vendor \r\n"
			+ " where inward.is_deleted = 0"
			+ " and case when :searchText is not null and LENGTH(:searchText) >0 then (vendor.vendor_name like %:searchText% or inward.inward_number like %:searchText% or inward.document_type like %:searchText% ) else 1=1 end " 
	...
	@Query(value = "SELECT inward.inward_id, inward.inward_number, inward.purpose_type, inward.vendor_id, inward.transporter_name, inward.transporter_phone_no, "
			...
			+ " vendor_name, (select item_name from trading_material_master mat where mat.item_id =  child.item_id) as itemName"
